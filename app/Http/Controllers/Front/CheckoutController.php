    ...
    public function checkout()
    {
        ...
        if (count($data["cart"]) > 0) {
            ...
            if (!empty($data["cart"])) {
                ...
                $data["sub_total"] = Cart::instance('cart')->subTotal();
//        -----------tax-------------
                $data['is_tax_enable'] = SM::get_setting_value("is_tax_enable", 0);
                $data['default_tax'] = SM::get_setting_value("default_tax", 0);
...
//                if (count($taxInfo) > 0) {
    ...
    public function couponCheck(Request $request)
    {
        ...
        if ($validator->passes()) {
            ...
            $response['check_coupon'] = 0;
            $response['title'] = $validator->errors()->all();
            $response['message'] = 'Description';
            return response()->json($response);
        }
    }
...
//            ]);
            ...
            $user->billing_name = $billing["billing_name"];
            ...
            if ($request->payment_method_id == 6) {
                ...
                $tran_id = 'mahmudmart-' . rand(10000, 20000);
                ...
                $url_request = 'https://securepay.easypayway.com/payment/request.php';
//                $url = 'http://sandbox.easypayway.com/payment/request.php';
                $fields = array(
                    ...
                    'signature_key' => 'f3a1b635f37523e26d2077fef8104ab0'
                );
...
//
                ...
                //  $domain = 'innerinside.com'; // or Manually put your domain name
                // $ip = '192.185.165.92';
...
//            }
            ...
            $order->gross_amount = $request->sub_total;
            $order->discount = $request->discount;
            $order->discount_type = 'fixed';
            $order->input_discount = $request->discount;
...
//            $order->created_by = SM::current_user_id();
            if ($order->save()) {
                ...
                $order_address->name = $shipping["name"];
                ...
                $order_address->billing_name = $billing["billing_name"];
            ...
            $contact_email3 = 'sumaiya.minnat@gmail.com';
...
//                \Mail::to($contact_email3)->queue(new InvoiceMail($order_id));
                $info['message'] = 'Mail Successfully Send';
            }
            return redirect('/dashboard/orders/detail/' . $order_id)->with('s_message', "Ordered Successfully!");
        }
        return redirect('/order-success')->with('s_message', "Ordered Successfully!");
    }
