...
trait SM_Front
{
    ...
    public static function create_uri($table, $title, $id = null, $count = null)
    {
        ...
        if ($slug == "") {
            ...
            $slug = $count === null ? $title : $title . $count;
        }
    ...
    public static function create_uri2($table, $title, $id = null, $count = null)
    {
        $slug = $count === null ? str_slug($title) : $title . $count;
        $slug = self::sm_sanitize_filename($slug);
        if ($id === null) {
        ...
        if (count($category_data) > 0) {
            foreach ($category_data as $s_category) {
                ...
                if (count($s_category->products) > 0) {
                    $html .= '<label style="margin-left:12px;" class="fil-check" for="c1_' . $s_category->id . '">' . $s_category->title . '
                            <input ' . $selected . ' type="checkbox" id="c1_' . $s_category->id . '"
                                   value="' . $s_category->id . '" class="common_selector category"/>
...
//                ->limit($noOfProduct)
    ...
    public static function addToCartButton($product_id = null, $regular_price = null, $sale_price = null, $addClass = null)
    {
        ...
        if (!empty($item)) {
            $add_to_cart_button = '<a href="javascript:void(0)"  data-product_id="' . $product_id . '" title="Add to cart" class="addToCart btn btn-warning button-absotalate">Add to cart</a>';
...
//                                    </div>';
            ...
            if ($product->product_qty > 0) {
                if ($product->product_type == 1) {
                    ...
                    $add_to_cart_button = '<a href="' . url('product/' . $product->slug) . '" class="btn btn-warning button-absotalate" title="Buy Now">Buy Now </a>';
                } else {
                    $add_to_cart_button = 'Button';
                }
            } else {
                $add_to_cart_button = '<a href="' . url('product/' . $product->slug) . '" class="btn btn-warning button-absotalate" title="Out of Stock">Out of Stock </a>';
            }
        }
...
//        $get_currency = DB::table('orders')->where('id', $order_id)->first();
    ...
    public static function SubMenuCategory($cat_slug)
    {
        $cat = Category::Published()->where('slug', $cat_slug)->select('id')->first();
        if (!empty($cat)) {
            $get_data = Category::Published()
                ->where('parent_id', $cat->id)
                ->where('is_home_menu', 1)
                ->select('title', 'slug')
                ->get();
        } else {
            $get_data = array();
...
//                return Tag::where("total_products", ">", 0)->get();
    ...
    public static function getProductBrands($isWithHaveNoPtoduct = 1)
    {
        if ($isWithHaveNoPtoduct == 0) {
    ...
    public static function getProductAuthors($isWithHaveNoPtoduct = 1)
    {
        if ($isWithHaveNoPtoduct == 0) {
            return SM::getCache('authors_have_products', function () {
                return Author::Published()->get();
            });
        } else {
            return SM::getCache('authors_have_not_product', function () {
                return Author::Published()->get();
            });
        }
    }

    public static function getProductPublishers($isWithHaveNoPtoduct = 1)
    {
        if ($isWithHaveNoPtoduct == 0) {
            return SM::getCache('publishers_have_products', function () {
                return Publisher::Published()->get();
            });
        } else {
            return SM::getCache('publishers_have_not_product', function () {
                return Publisher::Published()->get();
            });
        }
    }
