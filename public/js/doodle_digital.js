...
(function ($) {
            ...
            success: function (response) {
                if (id == 'forgotPassword') {
                    ...
                    if (response.username != undefined) {
                        var html = '<li><a href="' + url + '/dashboard" class="profile-img">' + response.username + '</a></li>\n' +
                            '<li><a href="' + url + '/logout"><i class="fa fa-sign-in"></i>Logout</a></li>' +
                            '<li><a class="searchIcon" href="#"><img src="' + url + 'images/search.png" alt="search"></a></li>';
                        $("#auth_ul").html(html);
                        $(".smAuthHide").fadeOut();
                    }
                }
    ...
    if ($('#orderForm').length > 0) {
        ...
        $(".applybtn").on("click", function () {
            var couponCode = $("#copon-code").val();
            var token = $('#table_csrf_token').val();
            if (couponCode.length > 0) {
                $.ajax({
                    type: 'post',
                    url: url + "checkout/coupon-check",
                    data: {_token: token, couponCode: couponCode},
                    success: function (successResponse) {
                        $("#copon-code").siblings(".error-notice").text("");
                        if (successResponse.isSuccess == 1) {
                            var total = parseFloat($("#sub_total").text());
                            var tax = parseFloat($("#tax").text());
                            var total_checkout = total;
                            if (successResponse.type == 1) {
                                total_checkout -= parseFloat(successResponse.discount);
                                $("#coupon_amount").text(successResponse.discount);
                            } else {
                                var discount = parseFloat(successResponse.discount);
                                discount = total_checkout * discount / 100;
                                $("#coupon_amount").text(discount);
                                total_checkout -= discount;
                            }
                            total_checkout += tax;
                            $("#total_checkout").text(total_checkout);
                            $("#coupon_code").text(successResponse.coupon_code);
                            $("#copon-code").siblings(".success-notice").text(successResponse.message);
                            $("#coupon_row").slideDown();
                        } else {
                            $("#copon-code").siblings(".error-notice").text(successResponse.message);
                        }
                    },
                    error: function (errorResponse) {
                        // console.log(errorResponse);
                        $("#copon-code").siblings(".success-notice").text("");
                        $("#copon-code").siblings(".error-notice").text(errorResponse.responseJSON.message);
                    }
                });
            }
            return false;
        ...
        /**
         * Tax processing country onChange
         */

        $("#country").on("change", function () {
            var $this = $(this),
                _token = $('#table_csrf_token').val(),
                country = $this.val(),
                is_tax_enable = parseInt($("#tax").data("is_tax_enable")),
                coupon_amount = parseFloat($("#coupon_amount").text()),
                sub_total = parseFloat($("#sub_total").text());
            // console.log("coupon_amount " + coupon_amount + " sub_total " + sub_total);
            if (is_tax_enable === 1 && country.length > 0) {
                $.ajax({
                    type: 'post',
                    url: url + 'checkout/calculate-tax',
                    data: {country: country, _token: _token},
                    success: function (successResponse) {
                        // console.log(successResponse);
                        if (successResponse.isSuccess == 1) {
                            var tax = parseFloat(successResponse.tax);
                            if (successResponse.tax_type == 2) {
                                tax = tax * sub_total / 100;
                            }
                            sub_total -= coupon_amount;
                            sub_total += tax;
                            // console.log("tax " + tax + " sub_total " + sub_total);
                            $("#tax").text(tax);
                            $("#total_checkout").text(sub_total);
                        }
                    },
                    error: function (errorResponse) {
                        console.log(errorResponse);
        ...
        $("#card-number").on('keyup', function () {
            cardValidationCheck(0);
        });
        $("#card-number").on('change', function () {
            cardValidationCheck(0);
        });
        $("#cvv2").on('keyup', function () {
            cvv2ValidationCheck(0);
        });
        $("#cvv2").on('change', function () {
            cvv2ValidationCheck(0);
        });
        // $("#card_expire").on('keyup', function () {
        //     cardExpireValidationCheck(0);
        // });
        // $("#card_expire").on('change', function () {
        //     cardExpireValidationCheck(0);
        // });
        /**
        ...
        $('#orderForm').on("submit", function () {
            ...
            var payment_type = parseInt($(".payment_type:checked").val());
            if (payment_type === 2 || payment_type === 3) {
                error = cardValidationCheck(error);
                error = cvv2ValidationCheck(error);
                error = cardExpireValidationCheck(error);
            }

            var contact_email = $("#contact_email").val();
            if (emailValidate(contact_email)) {
                $("#contact_email").siblings(".error-notice").text("");
            } else {
                error++;
                $("#contact_email").siblings(".error-notice").text("Email must be valid");
            }
            lenValidation("firstname", 0, error);
            lenValidation("lastname", 0, error);
            lenValidation("street", 0, error);
            lenValidation("city", 0, error);
            lenValidation("zip", 0, error);
            lenValidation("country", 0, error);
            lenValidation("state", 0, error);
            if ($("#agree").is(":checked")) {
                $("#agree").siblings(".error-notice").text("");
            } else {
                $("#agree").siblings(".error-notice").text("You must check terms and condition and Services Agreement .");
                error++;
            }
        ...
        $(".validateName").keyup(function (e) {
            var name =$(this).val();
            var nameFilter = /^[a-zA-Z .!?"-]+$/;
            if (!nameFilter.test(name)) {
                removeLastCharacter(name, $(this));
            }
        });
    ...
    /** Blog Comment Load more Pagination  */
    if ($(".loadMoreComments").length > 0) {
        $(".blog-comments-area").on("click", '.loadMoreComments', function () {
            var $this = $(this),
                id = $this.attr("id"),
                loaded = $this.data("loaded"),
                last = $this.data("last"),
                count = parseInt($this.data("count"))
            var loadingDefaultHtml = $this.html();
            $this.html('<i class="fa fa-spinner fa-spin"></i> Loading...');
            $.ajax({
                type: 'get',
                url: $this.data("url") + "/" + last,
                success: function (response) {
                    loaded += parseInt(response.commnetsCount);
                    $this.data("last", response.last);
                    $this.data("loaded", loaded);
                    if (count <= loaded) {
                        $("#" + id).fadeOut();
                    }
                    $this.parent(".ab-pagination-list").siblings(".comment-placeholder").append(response.html);
                    $this.html(loadingDefaultHtml);
                },
                error: function (error) {
                }
            });
     ...
     * like
     */
    if ($('.mrks_like').length > 0) {
        $('.mrks_like').on('click', function () {
            var $this = $(this),
                id = $this.data("id"),
                type = $this.data("type"),
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (response.isAlreadyLiked == 1 && !$this.hasClass('mrks_liked')) {
                        $this.addClass('mrks_liked')
                    } else {
                        $this.removeClass('mrks_liked')
                    }
                },
                error: function () {
