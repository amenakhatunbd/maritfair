...
function change_state(selectedCountryIndex, state) {
...
function change_s_state(s_selectedCountryIndex, s_state) {
    var s_stateElement = document.getElementById(s_state);
    s_stateElement.length = 0; // Fixed by Julian Woods
    s_stateElement.options[0] = new Option('Select State / Province', '');
    s_stateElement.selectedIndex = 0;
    var s_state_arr = s_a[s_selectedCountryIndex].split("|");
    for (var i = 0; i < s_state_arr.length; i++) {
        s_stateElement.options[s_stateElement.length] = new Option(s_state_arr[i], s_state_arr[i]);
    }
}
...
function change_country() {
    var country_onload = $('.country').data('onload');
    if (country_onload != '') {
        var state_onload = $('.state').data('onload');
        $('.country').val(country_onload);
        ...
        $('.state').val(state_onload);
    }
}
function change_s_country() {
    var country_onload = $('.s_country').data('onload');
    if (country_onload != '') {
        var state_onload = $('.s_state').data('onload');
        $('.s_country').val(country_onload);
...
//        console.log('ok');
        $('.country').on('change', function () {
            var selectedCountryIndex = $(this).find('option:selected').attr('data-id');
...
//        console.log('ok');
        $('.s_country').on('change', function () {
            var s_selectedCountryIndex = $(this).find('option:selected').attr('data-id');
            var s_state = $(this).attr('data-s_state');
            change_s_state(s_selectedCountryIndex, s_state);
        });
    }
});
